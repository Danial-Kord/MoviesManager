...
import { IconDatabase, IconFolder, IconSparkles, IconTrash, IconUpload } from "@/components/icons";
import { useLocale } from "@/lib/i18n/context";
import { interpolate, type MessageKey } from "@/lib/i18n/messages";

type SettingsTab = "overview" | "library" | "dubbed" | "database" | "maintenance";
...
export function SettingsClient() {
  ...
  const [dubbedRules, setDubbedRules] = useState<DubbedRuleDto[]>([]);
  const [dubbedLoaded, setDubbedLoaded] = useState(false);
  const [dubbedSaveBusy, setDubbedSaveBusy] = useState(false);
  ...
  const refreshDubbedRules = useCallback(async () => {
    try {
      const data = await fetchLibrarySettings();
      setDubbedRules(data.dubbedRules);
      setErr(null);
    } catch {
      setErr(t("settingsDubbedLoadErr"));
    } finally {
      setDubbedLoaded(true);
    }
  }, [t]);
  ...
  useEffect(() => {
    void refreshPaths();
    void refreshDbInfo();
    void refreshDubbedRules();
  }, [refreshPaths, refreshDbInfo, refreshDubbedRules]);
  ...
  async function onSaveDubbedRules() {
    setErr(null);
    setOk(null);
    setDubbedSaveBusy(true);
    try {
      const saved = await patchLibrarySettings({ dubbedRules });
      setDubbedRules(saved.dubbedRules);
      setOk(t("settingsDubbedSaveOk"));
    } catch (e) {
      setErr((e as Error).message);
    } finally {
      setDubbedSaveBusy(false);
    }
  }

  async function onRecomputeDubbed() {
    if (!window.confirm(t("settingsDubbedReapplyConfirm"))) return;
    setErr(null);
    setOk(null);
    setReapplyBusy(true);
    try {
      const r = await postRecomputeDubbed();
      setOk(interpolate(t("settingsDubbedReapplyDone"), { checked: r.rowsChecked, updated: r.rowsUpdated }));
    } catch (e) {
      setErr((e as Error).message);
    } finally {
  ...
  const tabs: { id: SettingsTab; labelKey: MessageKey }[] = [
    { id: "overview", labelKey: "settingsTabOverview" },
    { id: "library", labelKey: "settingsTabLibrary" },
    { id: "dubbed", labelKey: "settingsTabDubbed" },
    { id: "database", labelKey: "settingsTabDatabase" },
    { id: "maintenance", labelKey: "settingsTabMaintenance" },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <nav className="flex flex-wrap gap-2 border-b border-imdb-border pb-4" aria-label="Settings sections">
        {tabs.map(({ id, labelKey }) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-[12px] font-semibold transition ${
              tab === id
                ? "bg-imdb-gold text-black ring-2 ring-imdb-gold/40"
                : "bg-imdb-panel/70 text-imdb-muted ring-1 ring-imdb-border hover:bg-imdb-hover hover:text-imdb-text"
            }`}
          >
            {t(labelKey)}
          </button>
      ...
      <div role="tabpanel" className="space-y-10">
            ...
            <section className="rounded-imdb-card border border-imdb-border bg-gradient-to-b from-imdb-panel/25 via-imdb-elevated to-imdb-elevated p-6 shadow-md shadow-black/25 md:p-8">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <InfoTile title="Movies &amp; TV series">
                  <p>
                    One file per row in the database. Episode-shaped filenames are merged into a single{" "}
                    <strong className="font-semibold text-imdb-text">series</strong> card on Home; open it for seasons and episodes. Standalone files stay{" "}
                    <strong className="font-semibold text-imdb-text">movies</strong>.
                  </p>
                </InfoTile>
                <InfoTile title="Dubbed">
                  <p>{t("settingsOverviewDubbedTile")}</p>
                </InfoTile>
                <InfoTile title="Categories">
                  <p>
                    On a <strong className="font-semibold text-imdb-text">movie</strong> detail page you can assign{" "}
                    <strong className="font-semibold text-imdb-text">categories</strong> (tags). The{" "}
                    <strong className="font-semibold text-imdb-text">Genre</strong> filter on Home is a dropdown built from genres already stored on your titles (usually after TMDb enrichment).
                  </p>
                </InfoTile>
                ...
                <InfoTile title="Play">
                  <p>
                    <strong className="font-semibold text-imdb-text">Play</strong> calls the local API so your OS opens the file (VLC, Movies &amp; TV, etc.) — playback is not inside the browser.
                  </p>
                </InfoTile>
              </div>
            </section>
        ...
        {tab === "library" ? (
          ...
          >
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-[11px] font-semibold uppercase tracking-wider text-imdb-dim">Add folder</label>
        ...
        {tab === "dubbed" ? (
          <SettingsSection icon={<IconSparkles size={20} />} title={t("settingsDubbedSectionTitle")} description={t("settingsDubbedSectionDesc")}>
            <div className="space-y-4">
              <p className="text-[12px] leading-relaxed text-imdb-muted">{t("settingsDubbedBuiltinsNote")}</p>

              {!dubbedLoaded ? (
                <p className="text-[13px] text-imdb-muted">{t("loading")}</p>
              ) : (
                <>
                  <ul className="space-y-3">
                    {dubbedRules.length === 0 && (
                      <li className="rounded-imdb border border-dashed border-imdb-border bg-imdb-canvas/35 px-4 py-6 text-center text-[13px] text-imdb-subtle">
                        {t("settingsDubbedEmptyRules")}
                      </li>
                    )}
                    {dubbedRules.map((rule, index) => (
                      <li
                        key={index}
                        className="flex flex-col gap-3 rounded-imdb border border-imdb-border bg-imdb-canvas/45 p-4 md:flex-row md:flex-wrap md:items-center"
                      >
                        <input
                          className="min-w-[12rem] flex-1 rounded-imdb border border-imdb-border bg-imdb-canvas/90 px-3 py-2 font-mono text-[13px] text-imdb-text outline-none focus:border-imdb-focus focus:ring-2 focus:ring-imdb-focus/25"
                          value={rule.pattern}
                          onChange={(e) => {
                            const v = e.target.value;
                            setDubbedRules((prev) => prev.map((r, i) => (i === index ? { ...r, pattern: v } : r)));
                          }}
                          placeholder={t("settingsDubbedPatternPlaceholder")}
                          spellCheck={false}
                          autoComplete="off"
                          aria-label={t("settingsDubbedPatternPlaceholder")}
                        />
                        <select
                          className="rounded-imdb border border-imdb-border bg-imdb-elevated px-3 py-2 text-[13px] text-imdb-text outline-none focus:border-imdb-focus focus:ring-2 focus:ring-imdb-focus/25"
                          value={rule.mode}
                          onChange={(e) => {
                            const mode = e.target.value === "regex" ? "regex" : "contains";
                            setDubbedRules((prev) => prev.map((r, i) => (i === index ? { ...r, mode } : r)));
                          }}
                        >
                          <option value="contains">{t("settingsDubbedModeContains")}</option>
                          <option value="regex">{t("settingsDubbedModeRegex")}</option>
                        </select>
                        <label className="flex cursor-pointer items-center gap-2 text-[13px] text-imdb-text">
                          <input
                            type="checkbox"
                            checked={rule.enabled !== false}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setDubbedRules((prev) => prev.map((r, i) => (i === index ? { ...r, enabled: checked } : r)));
                            }}
                          />
                          {t("settingsDubbedEnabled")}
                        </label>
                        <button
                          type="button"
                          onClick={() => setDubbedRules((prev) => prev.filter((_, i) => i !== index))}
                          className="rounded-imdb px-3 py-2 text-[12px] font-semibold text-imdb-error ring-1 ring-imdb-error/35 transition hover:bg-imdb-error/10 md:ms-auto"
                        >
                          {t("settingsDubbedRemoveRow")}
                        </button>
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => setDubbedRules((prev) => [...prev, { pattern: "", mode: "contains", enabled: true }])}
                      className="rounded-imdb bg-imdb-panel px-4 py-2 text-[12px] font-semibold text-imdb-text ring-1 ring-imdb-border transition hover:bg-imdb-hover"
                    >
                      {t("settingsDubbedAddRule")}
                    </button>
                    <button
                      type="button"
                      disabled={dubbedSaveBusy}
                      onClick={() => void onSaveDubbedRules()}
                      className="rounded-imdb bg-imdb-gold px-5 py-2 text-[12px] font-semibold text-black transition hover:brightness-95 disabled:opacity-50"
                    >
                      {dubbedSaveBusy ? t("settingsDubbedSaving") : t("settingsDubbedSave")}
                    </button>
                    <button
                      type="button"
                      disabled={reapplyBusy || dubbedSaveBusy}
                      onClick={() => void onRecomputeDubbed()}
                      className="rounded-imdb border border-imdb-border bg-transparent px-4 py-2 text-[12px] font-semibold text-imdb-text transition hover:bg-imdb-hover disabled:opacity-50"
                    >
                      {reapplyBusy ? t("settingsDubbedReapplyBusy") : t("settingsDubbedReapply")}
                    </button>
                  </div>
                </>
              )}
